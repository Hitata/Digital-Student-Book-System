...
namespace ImageProcessing
{
	...
	{   
		 ...
		 */
		public override string Convert(string str, bool html) 
		{
			if (html) 
			{
				str = ConvertHTML(str);
...
//			str = str.Replace('\u0092', '\''); // replace right single quotation mark (\u0092, or \u2019) with apostrophe
			...
			str = Regex.Replace(str, "(?i)(?<=(?:(?:[aeiouy][`?~'.^(+]{1,2})|[ae])[aeiouy])(?=[?'.](?:\\s|$|\\p{P}))", @"\");
			...
			str = Regex.Replace(strB.ToString(), "(?i)\\\\(?=[-.?'d\\\\])", "");
		...
		*/
		string cleanupURL(string str)
		{
			...
			try
			{
				while ((foundIndex = strB.ToString().IndexOf("://", startIndex)) != -1)
				{
